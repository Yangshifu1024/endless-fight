...
import { damageAfterDefense, enemyAtkAtStage, enemyDefAtStage, enemyHpAtStage } from '../logic/balance'
...
import type { ActiveSkillId, EquipmentItem, PlayerSave, SkillId, UltimateSkillId } from '../model/types'
import { allSkills, canLearnOrUpgrade, canSelectBranch, getSkillDef, skillBranch, skillLevel } from '../skills/skills'
import { chainLightningParams, meteorParams, skillCooldownMs, skillPreviewLines, whirlwindParams } from '../skills/effects'
...
export class BattleScene extends Phaser.Scene {
  ...
  create() {
    ...
    this.playerCircle = this.add.circle(width * 0.5, height * 0.58, 13, 0xe2e8f0)
    this.playerHp = computeDerivedPlayerStats(this.save).hpMax
    this.createPlayerHpBar()

    this.uiLeft = this.add.text(12, 10, '', { fontFamily: 'system-ui', fontSize: '14px', color: '#e2e8f0' })
    this.uiRight = this.add
      .text(width - 12, 10, '', { fontFamily: 'system-ui', fontSize: '14px', color: '#e2e8f0', align: 'right' })
      .setOrigin(1, 0)
    this.uiBottom = this.add
      .text(12, height - 12, '', { fontFamily: 'system-ui', fontSize: '14px', color: '#94a3b8' })
      .setOrigin(0, 1)
  ...
  private createPlayerHpBar() {
    ...
    this.playerHpBarBg = this.add.rectangle(this.playerCircle.x, y, barW, barH, 0x0b1220, 0.92)
    this.playerHpBarBg.setStrokeStyle(1, 0x334155, 1)
    this.playerHpBarFill = this.add
      .rectangle(this.playerCircle.x - barW / 2 + 1, y, barW - 2, barH - 2, 0x22c55e, 1)
      .setOrigin(0, 0.5)
  ...
  private updatePlayerHpBar() {
    ...
    const pct = Phaser.Math.Clamp(derived.hpMax <= 0 ? 0 : this.playerHp / derived.hpMax, 0, 1)
    ...
    this.playerHpBarFill.setFillStyle(pct <= 0.3 ? 0xef4444 : pct <= 0.6 ? 0xf59e0b : 0x22c55e, 1)
  }
  ...
  private updatePlayer(dt: number) {
    ...
    const { width, height } = this.scale
    const nx = Phaser.Math.Clamp(this.playerCircle.x + vx * speed * (dt / 1000), 18, width - 18)
    const ny = Phaser.Math.Clamp(this.playerCircle.y + vy * speed * (dt / 1000), 40, height - 18)
  ...
  private updateCombat(dt: number) {
    ...
    for (const e of this.enemies) {
      const dist = Phaser.Math.Distance.Between(e.sprite.x, e.sprite.y, this.playerCircle.x, this.playerCircle.y)
      if (dist > 22) continue
      ...
      this.playerHp -= dmg
      this.spawnFloatText(this.playerCircle.x, this.playerCircle.y - 18, `-${Math.floor(dmg)}`, '#fca5a5')
      if (this.playerHp <= 0) {
        this.playerHp = 0
  ...
  private playerAttack(derived: ReturnType<typeof computeDerivedPlayerStats>) {
    ...
    for (const e of this.enemies) {
      const d = Phaser.Math.Distance.Between(e.sprite.x, e.sprite.y, this.playerCircle.x, this.playerCircle.y)
      if (d < bestDist) {
        bestDist = d
        best = e
      }
    ...
    best.hp -= dmg
    this.spawnFloatText(best.sprite.x, best.sprite.y - 16, `${crit ? '暴 ' : ''}${Math.floor(dmg)}`, crit ? '#fde68a' : '#e2e8f0')
  ...
  private updateSkillCasting(dt: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
  ...
  private castActiveSkill(id: ActiveSkillId, lv: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
    switch (id) {
      case 'whirlwind':
  ...
  private castUltimateSkill(id: UltimateSkillId, lv: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
    switch (id) {
      case 'meteor':
  ...
  private castWhirlwindSkill(lv: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
    ...
    const ring = this.add.circle(this.playerCircle.x, this.playerCircle.y, p.radius, 0x60a5fa, 0.12)
    this.tweens.add({
      targets: ring,
      alpha: 0,
      duration: 240,
    ...
    for (const e of hit) {
      ...
      e.hp -= dmg
      this.spawnFloatText(e.sprite.x, e.sprite.y - 16, `${Math.floor(dmg)}`, '#93c5fd')
  ...
  private castChainLightningSkill(lv: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
    ...
    while (current && chain.length < p.bounces) {
      ...
      current = this.findNearestEnemyTo(current.sprite.x, current.sprite.y, p.range, hitIds)
    }
    ...
    for (const e of chain) {
      ...
      e.hp -= dmg
      this.spawnFloatText(e.sprite.x, e.sprite.y - 16, `${Math.floor(dmg)}`, '#c4b5fd')
  ...
  private castMeteorSkill(lv: number, derived: ReturnType<typeof computeDerivedPlayerStats>) {
    const branch = skillBranch(this.save, 'meteor')
    const p = meteorParams(lv, branch)
    const raw = derived.atk * p.coef
    const targets = this.enemies.filter(
      (e) => Phaser.Math.Distance.Between(e.sprite.x, e.sprite.y, this.playerCircle.x, this.playerCircle.y) <= p.radius,
    )
    if (targets.length <= 0) return

    const ring = this.add.circle(this.playerCircle.x, this.playerCircle.y, p.radius, 0xf59e0b, 0.12)
    this.tweens.add({ targets: ring, alpha: 0, duration: 360, onComplete: () => ring.destroy() })

    for (const e of targets) {
      const dmg = damageAfterDefense(raw, e.def)
      e.hp -= dmg
      this.spawnFloatText(e.sprite.x, e.sprite.y - 16, `${Math.floor(dmg)}`, '#fdba74')
    ...
    if (p.burn) {
      ...
      this.time.delayedCall(650, () => {
        ...
        const laterTargets = this.enemies.filter(
          (e) =>
            Phaser.Math.Distance.Between(e.sprite.x, e.sprite.y, this.playerCircle.x, this.playerCircle.y) <= p.radius,
        )
        for (const e of laterTargets) {
          const dmg = damageAfterDefense(tickRaw, e.def)
          e.hp -= dmg
          this.spawnFloatText(e.sprite.x, e.sprite.y - 16, `${Math.floor(dmg)}`, '#fb923c')
  ...
  private findNearestEnemy() {
    return this.findNearestEnemyTo(this.playerCircle.x, this.playerCircle.y, Infinity, new Set())
  }

  private findNearestEnemyTo(x: number, y: number, maxDist: number, ignoreIds: Set<string>) {
    let best: Enemy | undefined
  ...
  private applyLifeSteal(derived: ReturnType<typeof computeDerivedPlayerStats>, dmgDealt: number) {
  ...
  private killEnemy(enemy: Enemy) {
    ...
    if (res.leveledUp > 0) {
      ...
      this.lastDropText = this.lastDropText ? `${this.lastDropText}；${msg}` : msg
    }
    ...
    if (drop.equipment) {
      ...
      const dropText = `掉落：${label}${drop.reforgeStone ? '，重铸石 +1' : ''}`
      this.lastDropText = this.lastDropText ? `${dropText}；${this.lastDropText}` : dropText
      this.spawnFloatText(enemy.sprite.x, enemy.sprite.y - 20, label, rarityColor((equipped ?? looted).rarity))
    }
  ...
  private sellValue(item: EquipmentItem) {
    const gold = Math.max(1, Math.floor(item.power * 0.12 + item.iLv * 3 + rarityRank(item.rarity) * 14))
  ...
  private spawnEnemy() {
    ...
    const pad = 10
    const x = edge === 0 ? -pad : edge === 1 ? width + pad : this.rng.int(0, width)
    const y = edge === 2 ? -pad : edge === 3 ? height + pad : this.rng.int(40, height)
    ...
    const sprite = this.add.circle(x, y, isElite ? 13 : 10, isElite ? 0xfb7185 : 0x94a3b8)
    const barW = isElite ? 34 : 28
    const barH = 6
    const offsetY = isElite ? 22 : 18
    const hpBarBg = this.add.rectangle(x, y - offsetY, barW, barH, 0x0b1220, 0.92)
    hpBarBg.setStrokeStyle(1, 0x334155, 1)
    const hpBarFill = this.add
      .rectangle(x - barW / 2 + 1, y - offsetY, barW - 2, barH - 2, isElite ? 0xf43f5e : 0x22c55e, 1)
      .setOrigin(0, 0.5)
    hpBarBg.setDepth(20)
  ...
  private createButtons() {
    const mk = (x: number, y: number, text: string, onClick: () => void) => {
      const t = this.add
        .text(x, y, text, { fontFamily: 'system-ui', fontSize: '14px', color: '#e2e8f0' })
  ...
  private openStageClearOverlay() {
    ...
    const { width, height } = this.scale
    const bg = this.add.rectangle(width * 0.5, height * 0.5, width, height, 0x000000, 0.55)
    const panel = this.add.rectangle(width * 0.5, height * 0.5, 520, 220, 0x0b1220, 0.95)
    ...
    const desc = this.add
      .text(width * 0.5, height * 0.5 - 34, this.save.autoNext ? '自动下一关已开启' : '选择：推进 or 留下刷', {
        fontFamily: 'system-ui',
        fontSize: '14px',
        color: '#94a3b8',
      })
    ...
    this.overlay = this.add.container(0, 0, [bg, panel, title, desc, nextBtn, stayBtn])
  ...
  private openDefeatOverlay() {
    ...
    const { width, height } = this.scale
    const bg = this.add.rectangle(width * 0.5, height * 0.5, width, height, 0x000000, 0.65)
    const panel = this.add.rectangle(width * 0.5, height * 0.5, 520, 220, 0x0b1220, 0.95)
    panel.setStrokeStyle(1, 0x334155, 1)
  ...
  private openBackpackOverlay() {
    ...
    const { width, height } = this.scale
    const bg = this.add.rectangle(width * 0.5, height * 0.5, width, height, 0x000000, 0.6)
    const panel = this.add.rectangle(width * 0.5, height * 0.5, 860, 480, 0x0b1220, 0.98)
    panel.setStrokeStyle(1, 0x334155, 1)

    const title = this.add
      .text(width * 0.5, height * 0.5 - 220, `装备背包  ${this.save.inventory.length}/80`, {
        fontFamily: 'system-ui',
        fontSize: '18px',
        color: '#e2e8f0',
      })
    ...
    const setEquipLine = (i: number, text: string, color = '#e2e8f0') => {
      ...
      const t =
        equipLines[i] ??
        this.add
          .text(leftX, y, '', { fontFamily: 'system-ui', fontSize: '13px', color: '#e2e8f0' })
          .setInteractive({ useHandCursor: true })
      t.setText(text)
      t.setColor(color)
      equipLines[i] = t
    ...
    const enhanceBtn = mkBtn(detailX + 120, '强化', () => {
      ...
      if (this.save.gold < cost.gold || this.save.essence < cost.essence) return
    ...
    const reforgeBtn = mkBtn(detailX + 190, '重铸', () => {
      ...
      if (this.save.gold < cost.gold || this.save.reforgeStone < cost.reforgeStone) return
      this.save.gold -= cost.gold
    ...
    const getSelectedItem = () => {
      ...
      if (selectedSource === 'inv') return this.save.inventory.find((it) => it.id === selectedId)
      const all = Object.values(this.save.equipment).filter(Boolean) as EquipmentItem[]
    ...
    const refresh = () => {
      ...
      money.setText(
        `金币 ${Math.floor(this.save.gold)}   精华 ${Math.floor(this.save.essence)}   重铸石 ${Math.floor(this.save.reforgeStone)}`,
      )

      for (let i = 0; i < slots.length; i++) {
        const slot = slots[i]!
        const it = this.save.equipment[slot.key]
        const label = it ? `${slot.label}：${formatItemShort(it)}` : `${slot.label}：空`
        const t = setEquipLine(i, label, it ? rarityColor(it.rarity) : '#94a3b8')
      ...
      const invSorted = [...this.save.inventory].sort((a, b) => b.power - a.power)
      ...
      for (let i = 0; i < pageSize; i++) {
        ...
        const it = slice[i]
        const text = it ? `${start + i + 1}. ${formatItemShort(it)}  强度 ${Math.floor(it.power)}` : ''
        const line =
          invLines[i] ??
          this.add
            .text(invX, y, '', { fontFamily: 'system-ui', fontSize: '12px', color: '#e2e8f0' })
            .setInteractive({ useHandCursor: true })
        invLines[i] = line
        line.setText(text)
        line.setColor(it ? rarityColor(it.rarity) : '#e2e8f0')
        line.setAlpha(it && selectedSource === 'inv' && selectedId === it.id ? 1 : it ? 0.9 : 1)
      ...
      actionInfo.setText(
        [
          selectedSource === 'equip' ? '已装备：可以强化/重铸，不能出售' : '背包：可以穿戴/强化/重铸/出售',
          `强化消耗：金币 ${ec.gold}，精华 ${ec.essence}${canEnhance(selected) ? '' : '（已满级）'}`,
          `重铸消耗：金币 ${rc.gold}，重铸石 ${rc.reforgeStone}`,
          selectedSource === 'inv' ? `出售获得：金币 ${this.sellValue(selected).gold}，精华 ${this.sellValue(selected).essence}` : '',
        ]
          .filter(Boolean)
          .join('\n'),
      )
  ...
  private openSkillsOverlay() {
    ...
    const points = this.add.text(leftX, topY, '', { fontFamily: 'system-ui', fontSize: '14px', color: '#e2e8f0' })
    ...
    const listTitle = this.add.text(leftX, topY + 60, '技能列表（升级得点；装配后自动施放）', {
      fontFamily: 'system-ui',
      fontSize: '13px',
      color: '#e2e8f0',
    })
    ...
    const equip1 = mkBtn(detailX, '装1', () => this.tryEquipSkillToSlot(selected, 0))
    const equip2 = mkBtn(detailX + 60, '装2', () => this.tryEquipSkillToSlot(selected, 1))
    const equip3 = mkBtn(detailX + 120, '装3', () => this.tryEquipSkillToSlot(selected, 2))
    const equipU = mkBtn(detailX + 180, '装U', () => this.tryEquipUltimate(selected))
    ...
    const refresh = () => {
      ...
      equipInfo.setText(
        `已装备：${this.formatActiveSlot(0)} / ${this.formatActiveSlot(1)} / ${this.formatActiveSlot(2)}   终极：${this.formatUltimateSlot()}`,
      )
      ...
      for (let i = 0; i < ids.length; i++) {
        ...
        const brName = br ? def.branches.find((b) => b.id === br)?.name ?? br : ''
        const brText = brName ? ` ${brName}` : ''
        const text = `${def.name}  Lv${lv}/${def.maxLevel}${brText} ${lockedText}`
        const t =
          skillLines[i] ??
          this.add
            .text(leftX, topY + 86 + i * 22, '', { fontFamily: 'system-ui', fontSize: '13px', color: '#e2e8f0' })
            .setInteractive({ useHandCursor: true })
        skillLines[i] = t
        t.setText(text)
      ...
      const branchLines =
        def.branches.length > 0 ? def.branches.map((b) => `${b.name}${b.description ? `：${b.description}` : ''}`) : []
      ...
      branchA.setText(def.branches[0] ? `分支：${def.branches[0].name}` : '分支A')
      branchB.setText(def.branches[1] ? `分支：${def.branches[1].name}` : '分支B')
  ...
  private updateUi() {
    ...
    const hpPct = derived.hpMax <= 0 ? 0 : (this.playerHp / derived.hpMax) * 100

    this.uiLeft.setText(
      [
        `关卡：${this.save.stage}  (重复 ${this.save.stageRepeat})`,
        `目标：${this.kills}/${this.killsNeeded}`,
        `等级：${this.save.level}  EXP：${this.save.exp}/${req} (${expPct.toFixed(1)}%)`,
        `金币：${Math.floor(this.save.gold)}`,
        `精华：${Math.floor(this.save.essence)}  重铸石：${Math.floor(this.save.reforgeStone)}`,
    ...
    this.uiRight.setText(
      [
        `HP：${Math.floor(this.playerHp)}/${Math.floor(derived.hpMax)} (${hpPct.toFixed(0)}%)`,
        `ATK：${Math.floor(derived.atk)}  DEF：${Math.floor(derived.def)}`,
        `攻速：${(1000 / derived.attackIntervalMs).toFixed(2)}/s`,
        `暴击：${(derived.critChance * 100).toFixed(1)}%  x${derived.critDamage.toFixed(2)}`,
        derived.lifeStealPct > 0 ? `吸血：${(derived.lifeStealPct * 100).toFixed(1)}%` : '',
      ]
        .filter(Boolean)
        .join('\n'),
    )
