...
export function chainLightningParams(lv: number, branch: string | undefined) {
  let bounces = 3 + Math.floor(lv / 2)
  let coef = 0.6 * skillDamageMult(lv)
  const range = 230
  let eliteMult = 1
  if (branch === 'fork') {
    bounces += 1
  ...
  return { bounces, coef, range, eliteMult }
}
...
export function skillPreviewLines(save: PlayerSave, id: SkillId) {
  ...
  if (id === 'whirlwind') {
    ...
    lines.push(`弹射：${p.bounces} 次`)
    lines.push(`范围：${p.range}`)
    lines.push(`倍率：${Math.round(p.coef * 100)}% ATK`)
...
function formatBranch(def: { branches: Array<{ id: string; name: string }> }, branchId: string | undefined) {
  if (!def.branches.length) return '无'
  if (!branchId) return `未选择（${def.branches.map((b) => b.name).join(' / ')}）`
